...
- nvmは非ログインシェルで`.bashrc`を読み込まないため、`wsl -d <ディストリ名> --`での実行時は`BASH_ENV`経由で明示的にnvmスクリプトをsourceする
...
- `wsl --import`時のディストリ名は汎用的な`Ubuntu`ではなく専用名（`BlueLamp`）を使う。利用者が既に別目的でUbuntu(WSL)を使っている場合、名前が衝突すると誤って既存環境を「導入済み」と誤認し、そちらに手を加えてしまう事故につながるため
